...
const dateForUnitEvents = require('../common/constants/index');
...
function generateUnitEvents(commonData, units) {
  ...
  units.forEach((unit) => {
    ...
    if (unit['$']['ScheduleStatus'] === 'UNSCHEDULED' ||
    unit['$']['ScheduleStatus'] === 'CANCELLED' ||
    unit['$']['ScheduleStatus'] === 'POSTPONED'){
      unitEventBody.startDate = dateForUnitEvents.startDate;
      unitEventBody.endDate = dateForUnitEvents.endDate;
    }
    if (unit['$']['HideStartDate'] === 'Y'){
      unitEventBody.eventBody.startDateTime = undefined;
      unitEventBody.startDate = dateForUnitEvents.startDate;
    }
    if (unit['$']['HideEndDate'] === 'Y'){
      unitEventBody.endDate = dateForUnitEvents.endDate;
    }
